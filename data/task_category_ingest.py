...
실행 방법 (ieum_ai 폴더 루트에서):
    python data/task_category_ingest.py
이미 같은 (department_code, task_id) 조합이 있으면 덮어씁니다(upsert).
"""
import sys
import os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from app.vectorstore.chroma_client import add_task_category
from data.seed_task_categories import TASK_CATEGORIES


def main():
    print(f"총 {len(TASK_CATEGORIES)}건 색인을 시작합니다...")
    for t in TASK_CATEGORIES:
        add_task_category(t["task_id"], t["name"], t["department_code"], t["description"])

    from collections import Counter
    counts = Counter(t["department_code"] for t in TASK_CATEGORIES)
    print("\n부서별 색인 건수:")
    for dept, cnt in sorted(counts.items()):
        print(f"  {dept}: {cnt}건")
    print(f"\n완료! 총 {len(TASK_CATEGORIES)}건이 ChromaDB(task_categories 컬렉션)에 색인되었습니다.")
