...
>
> **`similarity`와 실제 정렬 순서가 다를 수 있음**: 목록의 정렬은 `similarity`(임베딩 유사도)가 아니라 `rerank_score`(리랭커 점수) 기준입니다. 리랭커가 임베딩 유사도보다 더 정확한 관련성 판단을 한다고 보고 의도적으로 그렇게 설계했습니다 — 화면에는 `similarity` %만 참고용으로 보여주면 됩니다(예: 69%짜리가 72%짜리보다 위에 나올 수 있음, 버그 아님).
...
> **응답 지연**: 실측 최대 2분 19초 (HTML 마크다운 지시문이 추가되며 프롬프트가 길어져 기존 103초보다 늘어남). `ieum_ai` 내부 Ollama 호출 타임아웃이 240초로 설정되어 있으므로, 백엔드가 `ieum_ai`를 호출할 때 쓰는 타임아웃은 **반드시 240초보다 길게**(예: 280초 이상) 잡을 것 — 짧게 잡으면 `ieum_ai`가 아직 응답 중인데 백엔드가 먼저 포기해서 502로 보일 수 있음.
>
